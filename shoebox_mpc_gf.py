...
def model(shoebox, temperature_supply, temperature_outside, delta_time):
    shoebox.timestep(temperature_supply, temperature_outside, delta_time=delta_time)
...
def mpc_inner(actuation_strategy, shoebox, T_setpoint, temperature_outside, prediction_horizon, control_horizon, delta_time):
    ...
    for k in range(control_horizon):
        ...
        T_predicted = model(shoebox_copy, actuation_strategy[k], temperature_outside, delta_time=delta_time)
    ...
    for k in range(control_horizon, prediction_horizon):
        ...
        T_predicted = model(shoebox_copy, actuation_strategy[-1], temperature_outside, delta_time=delta_time)
        T_history[k + 1] = T_predicted
...
def mpc_outer(num_timesteps, initial_control, bounds, shoebox, T_setpoint,
    ...
    for i in range(num_timesteps):
        ...
        optimal_control_actions.append(result.x[0])  # take the first control action
