...
class DQNAgent:
    ...
    def _build_model(self):
        max_temp = 40
        min_temp = 0
        q_table = np.zeros((10 * (max_temp - min_temp), self.action_size))
        return q_table
    ...
    def act(self, state):
        ...
        state_index = min(int(state * 10), self.model.shape[0] - 1)
        act_values = self.model[state_index, :]
    ...
    def replay(self):
        ...
        for state, action, reward, next_state in minibatch:
            state_index = min(int(state * 10), self.model.shape[0] - 1)
            target = self.model[state_index, :]
            # target = self.model.predict(state[np.newaxis], verbose=0)
            # t = self.target_model.predict(next_state[np.newaxis], verbose=0)
            next_state_index = int(next_state * 10)
            t = self.model[next_state_index, :]
            # t = self.target_model.predict(next_state[np.newaxis], verbose=0)
            target[action] = reward + self.gamma * np.amax(t)
            # self.model.fit(state[np.newaxis], target, epochs=1, verbose=0)
            self.model[state_index, action] += self.learning_rate * (target[action] - self.model[state_index, action])
...
def training(num_layers, neurons_per_layer, file_model="chatgpt_q_building.keras", num_episodes=300, learning_rate=0.001):
    ...
    pickle.dump(agent.model, open(file_model, "wb"))
    # agent.model.save(file_model)


def test_model(num_layers, neurons_per_layer, file_model="chatgpt_q_building_model.pickle"):
    env = SmartBuildingEnv()
    state_size = env.reset().shape[0]
    action_size = len(env.heating_levels)
    agent = DQNAgent(state_size, action_size)
    agent.model = pickle.load(open(file_model, "rb"))
    ...
    for line in range(agent.model.shape[0]):
        print(f"T: {line / 10}, actions: {agent.model[line, :]}, optimal action: {np.argmax(agent.model[line, :])}")
    state = env.reset()
    for i in range(24):
...
def main():
    ...
    neurons_per_layer = 3
    set_global_seed()
    training(num_layers, neurons_per_layer, num_episodes=10000, file_model="chatgpt_q_building_model.pickle")
    test_model(num_layers, neurons_per_layer, file_model="chatgpt_q_building_model.pickle")
